import { Buffer } from "node:buffer";
import {
  Environment,
  SignedDataVerifier,
  type JWSRenewalInfoDecodedPayload,
  type JWSTransactionDecodedPayload,
  type ResponseBodyV2DecodedPayload,
} from "npm:@apple/app-store-server-library@3.1.0";

export const FIREVAULT_PRODUCT_IDS = new Set([
  "us.bannerman.firevault.technician.monthly",
  "us.bannerman.firevault.technician.annual",
]);

// Public DER roots downloaded from https://www.apple.com/certificateauthority/.
// SHA-256:
// Apple Inc. Root: b0b1730ecbc7ff4505142c49f1295e6eda6bcaed7e2c68c5be91b5a11001f024
// Apple Root CA G2: c2b9b042dd57830e7d117dac55ac8ae19407d38e41d88f3215bc3a890444a050
// Apple Root CA G3: 63343abfb89a6a03ebb57e9b3f5fa7be7c4f5c756f3017b3a8c488c3653e9179
const APPLE_ROOT_CERTIFICATES = [
  "",
  "",
  "",
].map((certificate) => Buffer.from(certificate, "base64"));

export type FireVaultSubscriptionStatus =
  | "active"
  | "grace_period"
  | "billing_retry"
  | "expired"
  | "revoked";

export type FireVaultSubscriptionState = {
  status: FireVaultSubscriptionStatus;
  expiresAt: string | null;
  gracePeriodExpiresAt: string | null;
  revokedAt: string | null;
  autoRenewEnabled: boolean | null;
};

export function deriveSubscriptionState(
  transaction: Pick<JWSTransactionDecodedPayload, "expiresDate" | "revocationDate">,
  renewal: Pick<JWSRenewalInfoDecodedPayload, "gracePeriodExpiresDate" | "isInBillingRetryPeriod" | "autoRenewStatus"> | null,
  now = Date.now(),
): FireVaultSubscriptionState {
  const expiresAt = isoDate(transaction.expiresDate);
  const gracePeriodExpiresAt = isoDate(renewal?.gracePeriodExpiresDate);
  const revokedAt = isoDate(transaction.revocationDate);
  const autoRenewEnabled = renewal?.autoRenewStatus === undefined
    ? null
    : renewal.autoRenewStatus === 1;

  if (revokedAt) {
    return { status: "revoked", expiresAt, gracePeriodExpiresAt, revokedAt, autoRenewEnabled };
  }
  if (renewal?.gracePeriodExpiresDate && renewal.gracePeriodExpiresDate > now) {
    return { status: "grace_period", expiresAt, gracePeriodExpiresAt, revokedAt, autoRenewEnabled };
  }
  if (transaction.expiresDate && transaction.expiresDate > now) {
    return { status: "active", expiresAt, gracePeriodExpiresAt, revokedAt, autoRenewEnabled };
  }
  if (renewal?.isInBillingRetryPeriod) {
    return { status: "billing_retry", expiresAt, gracePeriodExpiresAt, revokedAt, autoRenewEnabled };
  }
  return { status: "expired", expiresAt, gracePeriodExpiresAt, revokedAt, autoRenewEnabled };
}

export async function verifyTransaction(
  signedTransaction: string,
): Promise<{ transaction: JWSTransactionDecodedPayload; environment: "Production" | "Sandbox" }> {
  const production = productionVerifier();
  if (production) {
    try {
      return {
        transaction: await production.verifyAndDecodeTransaction(signedTransaction),
        environment: "Production",
      };
    } catch {
      // TestFlight and StoreKit testing transactions use the sandbox verifier.
    }
  }
  return {
    transaction: await sandboxVerifier().verifyAndDecodeTransaction(signedTransaction),
    environment: "Sandbox",
  };
}

export async function verifyNotification(
  signedPayload: string,
): Promise<{ notification: ResponseBodyV2DecodedPayload; environment: "Production" | "Sandbox" }> {
  const production = productionVerifier();
  if (production) {
    try {
      return {
        notification: await production.verifyAndDecodeNotification(signedPayload),
        environment: "Production",
      };
    } catch {
      // Continue with the sandbox verifier for TestFlight notifications.
    }
  }
  return {
    notification: await sandboxVerifier().verifyAndDecodeNotification(signedPayload),
    environment: "Sandbox",
  };
}

export async function verifyRenewalInfo(
  signedRenewalInfo: string | undefined,
  environment: "Production" | "Sandbox",
): Promise<JWSRenewalInfoDecodedPayload | null> {
  if (!signedRenewalInfo) return null;
  const verifier = environment === "Production" ? productionVerifier() : sandboxVerifier();
  if (!verifier) throw new Error("APPLE_APP_ID is required for production verification");
  return await verifier.verifyAndDecodeRenewalInfo(signedRenewalInfo);
}

function sandboxVerifier(): SignedDataVerifier {
  return new SignedDataVerifier(
    APPLE_ROOT_CERTIFICATES,
    true,
    Environment.SANDBOX,
    "us.bannerman.firevault",
  );
}

function productionVerifier(): SignedDataVerifier | null {
  const appAppleID = Number(Deno.env.get("APPLE_APP_ID"));
  if (!Number.isSafeInteger(appAppleID) || appAppleID <= 0) return null;
  return new SignedDataVerifier(
    APPLE_ROOT_CERTIFICATES,
    true,
    Environment.PRODUCTION,
    "us.bannerman.firevault",
    appAppleID,
  );
}

function isoDate(milliseconds: number | undefined): string | null {
  return milliseconds && Number.isFinite(milliseconds)
    ? new Date(milliseconds).toISOString()
    : null;
}
